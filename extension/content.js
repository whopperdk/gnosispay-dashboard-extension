...
function getMccCategory(mccCode) {
  ...
  for (const group of mccCategories) {
    for (const item of group.ranges) {
      ...
      if (typeof item === 'object' && code >= item.start && code <= item.end) {
        return group.category;
      }
    }
  }
...
function parseTransactionDate(dateText) {
  try {
    ...
    const timeMatch = dateText.match(/(\w+\s+\d{1,2})\s+at\s+(\d{1,2}:\d{2}\s+[AP]M)/i);
    if (timeMatch) {
      const datePart = timeMatch[1];
      const timePart = timeMatch[2];
      const parsedDate = new Date(`${datePart} ${currentYear} ${timePart}`);
      if (!isNaN(parsedDate.getTime())) {
        const parsedMonth = parsedDate.getMonth();
        if (parsedDate > now && parsedMonth > currentMonth) {
          parsedDate.setFullYear(currentYear - 1);
        } else if (parsedMonth <= currentMonth && parsedDate > now) {
...
async function scrapeTransactionsFromTable() {
  ...
  rows.forEach((row, index) => {
    ...
    let mcc = '0000';
    try {
      if (mccText && /\d{4}/.test(mccText)) {
        mcc = mccText.match(/(\d{4})/)[1];
      }
    } catch (error) {}
...
.mcc-cell div { 
  ...
  async function createSpendingChart(transactions) {
    ...
    const waitForContainer = () => {
      return new Promise((resolve) => {
        const checkContainer = () => {
          ...
          const table = document.querySelector('table, [class*="table"], [class*="transactions"], [role="grid"], [class*="data"], [class*="list"]');
          if (transactionHeader || table) {
            resolve({ transactionHeader, table });
          } else {
    ...
    if (transactionHeader && transactionHeader.parentNode) {
      transactionHeader.parentNode.insertBefore(container, transactionHeader.nextElementSibling);
    } else if (table) {
      table.parentNode.insertBefore(container, table);
    } else {
      document.body.appendChild(container);
        ...
        const checkControls = () => {
          if (
            ...
            if (attempts >= maxAttempts) {
              reject(new Error('Chart controls not found or not populated'));
            } else {
    ...
    function setupVisibilityToggles() {
      ...
      if (monthlyChartButton) {
        monthlyChartButton.addEventListener('click', () => {
          toggleButton(monthlyChartButton, monthlyChartWrapper, chartControls);
        });
      }

      if (yearlyChartButton) {
        yearlyChartButton.addEventListener('click', () => {
          toggleButton(yearlyChartButton, yearlyChartWrapper);
        });
      }

      if (cashbackCalculatorButton) {
        cashbackCalculatorButton.addEventListener('click', () => {
          toggleButton(cashbackCalculatorButton, cashbackCalculatorWrapper);
        });
      }
    ...
    function calculateCashbackRate(gnoAmount, hasOgNft) {
      let baseRate = 0;
      if (gnoAmount >= 0.1 && gnoAmount < 1) {
        baseRate = 1 + ((gnoAmount - 0.1) / (1 - 0.1)) * (2 - 1);
      } else if (gnoAmount >= 1 && gnoAmount < 10) {
        baseRate = 2 + ((gnoAmount - 1) / (10 - 1)) * (3 - 2);
      } else if (gnoAmount >= 10 && gnoAmount < 100) {
        baseRate = 3 + ((gnoAmount - 10) / (100 - 10)) * (4 - 3);
      } else if (gnoAmount >= 100) {
        baseRate = 4;
      }
    ...
    function calculateCashback(transactions, weekPeriod, gnoAmount, hasOgNft) {
      ...
      let to = null;
      if (weekPeriod) {
        const [start, end] = weekPeriod.split('|').map(d => new Date(d));
        from = start;
        to = end;
      }

      const filteredTransactions = transactions.filter(tx => {
        const dateField = tx.clearedAt;
        if (!dateField) return false;
        const txDate = new Date(dateField);
        if (isNaN(txDate.getTime())) return false;
        const isInDateRange = (!from || txDate >= from) && (!to || txDate <= to);
    ...
    function updateTableCashbackHighlights(filteredTransactions) {
      ...
      rows.forEach((row) => {
        ...
        const isEligibleMcc = mcc && !NO_CASHBACK_MCCS.includes(mcc);
        const isEligibleType = typeText && !['ATM_WITHDRAWAL', 'MONEY_TRANSFER', 'REFUNDED'].includes(typeText);
        if (!isEligibleMcc || !isEligibleType) return;

        const txDateObj = new Date(txDate);
        const isIncluded = filteredTransactions.some(tx => {
          const clearedDate = new Date(tx.clearedAt);
          const isSameDate =
            clearedDate.getFullYear() === txDateObj.getFullYear() &&
            clearedDate.getMonth() === txDateObj.getMonth() &&
            clearedDate.getDate() === txDateObj.getDate();
          const isSameMerchant = tx.merchant?.name?.trim() === merchantText;
          const isSameAmount = Math.abs(parseFloat(tx.billingAmount) - billingAmount) < 0.01;
    ...
    async function updateChart(transactionsToUse = allTransactions) {
      ...
      allCategories.forEach((category, i) => {
        pieCategoryColors[category] = colors[i];
      });
      ...
      if (monthlyTransactions.length) {
        ...
        allCategories.forEach((category, i) => {
          ...
          legendItem.addEventListener('click', () => {
            toggleHighlight(category, colors[i], month, year, monthlyTransactions, 'pie');
          });
    ...
    function calculateNiceTicks(maxValue, desiredTicks = 5) {
      ...
      if (ticks.length > desiredTicks + 1) {
        ticks.pop();
      }
    ...
    async function updateYearlyHistogram(transactionsToUse = allTransactions) {
      ...
      allCategories.forEach((category, i) => {
        histogramCategoryColors[category] = colors[i];
      });
      ...
      allCategories.forEach((category, i) => {
        ...
        `;
        legendItem.addEventListener('click', () => {
          toggleHighlight(category, colors[i], null, null, validTransactions, 'histogram');
        });
        legendContainer.appendChild(legendItem);
      });
    ...
    function toggleHighlight(category, color, selectedMonth, selectedYear, transactions, chartType) {
      ...
      if (selectedHighlights.has(highlightKey)) {
        selectedHighlights.delete(highlightKey);
      } else {
        selectedHighlights.add(highlightKey);
      }
    ...
    function updateTableHighlights(transactions, chartType, selectedCategory, selectedColor, selectedMonth, selectedYear) {
      ...
      rows.forEach((row) => {
        ...
        // Check if the row's transaction matches the filtered transactions
  ...
  async function convertToCSV(transactions) {
    ...
    const rows = transactions.map(tx => {
      const billingAmount = parseFloat(tx.billingAmount || 0).toFixed(2);
      return [
  ...
  async function getTransactions() {
    try {
      ...
      const mappedTransactions = transactions.map(tx => {
        const currencySymbol = currencyMap[tx.billingCurrency?.symbol] || '£';
        return {
